...
def add_features(data):
    ...
    data['total_occupancy'] = data.drop('srch_adults_count') + data['srch_children_count']
    data['family'] = (data.drop('srch_children_count') > 0).astype(int)
    # data['total_price_stay_sqrt'] = np.sqrt((data['price_usd'] * data['srch_length_of_stay']))
    data['occupants_per_room'] = data['total_occupancy'] / data.drop('srch_room_count')
...
def drop_columns(data, user_info):
    print("----------Dropping columns----------\n")
    columns_to_drop = user_info + ['comp1_rate', 'comp2_rate', 'comp3_rate', 'comp4_rate', 'comp5_rate', 'comp6_rate', 'comp7_rate', 'comp8_rate'] +\
                    ...
                    'comp5_rate_percent_diff', 'comp6_rate_percent_diff', 'comp6_rate_percent_diff', 'comp8_rate_percent_diff'] +\
                    ["price_usd", "srch_saturday_night_bool", "prop_log_historical_price"]
...
def main():
    ...
    train_data, test_data = drop_columns(add_features(train_data), user_info), drop_columns(add_features(test_data), user_info)
